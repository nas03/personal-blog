...
const sendMail = async (to, subject, text, htmlContent, settingOption, attachments = undefined) => {
  try {
...
const sendMailFXSignupXEM = async (to, subject, text, htmlContent, settingOption, attachments = undefined) => {
  try {
    const env = await _getEnvConfig([
      envCommon.EMAIL,
      envCommon.PASSWORD_EMAIL,
