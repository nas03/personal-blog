...
const { code, message, dateFormat, characterKatakana, resCheck, commonSiteId, displayDateTime, color,
  verificationStatusSiftApi, eventTypeSiftApi, resultDetailMessages, timeExpires, statusCode } = require('constant')
...
const setTokenForRedirectSite = (data) => {
  return jwt.sign(
    ...
    data.site_id === commonSiteId.MY_FOREX ? process.env.JWT_SECRET_MYFOREX : process.env.JWT_SECRET_FXON,
    {
      expiresIn: timeExpires.JWT,
    },
  )
}
...
const checkValidIP = (allow_list, deny_list) => {
  // CHECK LIST IP
  let isAllowValid = true
  let isDenyValid = true
  if (allow_list) {
    allow_list.split(',').forEach((item) => {
      if (!regex.ipAddressValidator(item.trim()) && !regex.ipAddressCIDRValidator(item.trim())) {
        isAllowValid = false
      }
    })
  }
